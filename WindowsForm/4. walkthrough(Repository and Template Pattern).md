...
- [Changes Made](#changes-made)
    ...
    - [3. Decoupling: CustomerEntity vs Domain Customer](#3-decoupling-customerentity-vs-domain-customer)
    - [4. Implemented Constructor Injection (DI)](#4-implemented-constructor-injection-di)
    - [5. Configured Composition Root](#5-configured-composition-root)
...
### 3. Decoupling: CustomerEntity vs Domain Customer
In the `Repository` project, we introduced a `CustomerEntity` class. This class is **internal** to the Repository layer and is used specifically for database mapping.

> [!NOTE]
> **Why we use `CustomerEntity` instead of the existing `Customer`:**
> - **Separation of Concerns**: The `Customer` and `Lead` classes in the `MiddleLayer` contain business logic and validation. The `CustomerEntity` is just a simple data structure for the database.
> - **Zero Dependency**: By using an internal entity, the `Repository` project **does not need a reference to the MiddleLayer**. It only knows about the `InterfaceLayer` and its own internal entities.
> - **Database Stability**: If the database table structure changes (e.g., a column name changes), you only update `CustomerEntity` and the mapping logic within the Repository. Your business logic in the `MiddleLayer` remains untouched.
> - **Null Handling**: Databases often have "junk" or "null" data (like a null `CustomerType`). We can handle these "dirty" data issues inside `CustomerEntity` and the Repository mapping (providing defaults like `"Customer"`) so the rest of the application always receives a "clean" and validated domain object.

### 4. Implemented Constructor Injection (DI)
